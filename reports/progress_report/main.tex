...
% 2. New Folder → figures/  and upload all fig_*.png / fig_*.jpg:
%      fig_pipeline.png, fig_hazard_overlay.jpg, fig_accuracy.png,
%      fig_ablations.png, fig_confusion_matrix.png, fig_uncertain_reasons.png,
...
Figure~\ref{fig:pipeline} summarizes the end-to-end architecture. The pipeline has five stages: detection, tracking, motion-feature extraction, hazard classification, and alert generation.
...
The alert module maps hazard labels to spoken messages (e.g.\ ``Bike approaching from the left.''). For \texttt{uncertain} tracks the message quotes the \texttt{uncertain\_reason}: ``Person detected briefly; not enough frames to judge motion'' for \texttt{short\_track}. Annotated frames and MP4 demos overlay bbox color, hazard label, and evidence text; videos are encoded with H.264 for macOS QuickTime compatibility (Figure~\ref{fig:overlay}).

\begin{figure*}[t]
\centering
\includegraphics[width=0.92\textwidth]{fig_pipeline.png}
\caption{End-to-end SpatialLens Assist pipeline. Each track receives a hazard label and an evidence string; ego-motion compensation is applied to optical flow and frame differencing before classification.}
\label{fig:pipeline}
\end{figure*}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_hazard_overlay.jpg}
\caption{Example hazard overlay on IMG\_4976: approaching pedestrians are highlighted with red banners; evidence text is printed below each bounding box.}
\label{fig:overlay}
\end{figure}
...
Table~\ref{tab:perclass} reports per-class metrics on the aggregate labeled set. Figure~\ref{fig:accuracy} shows headline accuracy; Figure~\ref{fig:ablations} and Table~\ref{tab:ablation} summarize ablations. The safety-critical \texttt{approaching} class---the primary collision-warning category---achieves F1 0.91 at precision 0.95.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_accuracy.png}
\caption{Decidable accuracy (tracks with $\ge$3 frames) vs.\ overall accuracy including deliberate \texttt{short\_track} abstentions. Macro F1 = 0.671; \texttt{approaching} precision/recall = 0.95/0.88.}
\label{fig:accuracy}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_ablations.png}
\caption{Ablation study: raising \texttt{max\_frame\_gap} is the largest gain; removing the v2 \texttt{strong\_crossing} rule drops accuracy to 78.9\,\%.}
\label{fig:ablations}
\end{figure}

\subsection{Failure Analysis}

Figure~\ref{fig:confusion} and Figure~\ref{fig:uncertain} decompose remaining errors.
The \texttt{approaching} column in the confusion matrix is clean; most off-diagonal mass flows to \texttt{uncertain} on 1--2 frame tracker fragments.
\texttt{short\_track} accounts for 98\,\% of uncertain predictions (39/40), indicating tracker fragmentation rather than classifier confusion as the dominant failure mode.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_confusion_matrix.png}
\caption{Aggregate confusion matrix (rows = ground truth, columns = predicted).}
\label{fig:confusion}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_uncertain_reasons.png}
\caption{Breakdown of \texttt{uncertain\_reason} sub-labels across all videos. \texttt{short\_track} dominates, motivating post-hoc track merging.}
\label{fig:uncertain}
\end{figure}
...
Figure~\ref{fig:trajectories} shows centroid paths from IMG\_4974 used for crossing detection. Uncontrolled clip IMG\_4989 exposes the remaining limit: $\sim$50\,\% of tracks fall to \texttt{uncertain/short\_track} when objects pass through the frame in only 1--2 sampled frames.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_trajectories.png}
\caption{Track trajectories in image coordinates (IMG\_4974).}
