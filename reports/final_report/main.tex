...
% --------------------------------------------------------------------------
The motivation for this work is personal. A few months ago my own
glasses broke, and even the short experience of crossing campus with
badly degraded vision made it strikingly hard to judge whether an
approaching cyclist or pedestrian was on a collision course with me.
For blind and low-vision pedestrians that difficulty is not a temporary
inconvenience but a constant, and far more severe, daily reality. That
gap is what motivated SpatialLens.
...
\noindent\emph{Scope (read first).} These numbers come from a single
small set --- 61 tracks across 11 videos, one author, one campus under
daylight --- with \emph{no held-out train/test split}. We report them
as evidence that the proposed motion cues are promising, not as a
deployment-grade claim. To avoid fitting the evaluation set, we
iterated only on rule \emph{structure} after each error-analysis pass
and never tuned a numeric threshold against these labels (all constants
are fixed in \texttt{config.yaml}). The two reporting denominators
(\cref{sec:dataset}) and the per-class macro-F1 are included precisely
so the small-sample weaknesses are visible rather than hidden. This
data regime is also \emph{why} the system is rule-based rather than
learned: with 61 labelled tracks and no held-out split, a neural
classifier or trajectory model---which typically needs thousands of
trajectories~\cite{trajectronpp,socialstgcnn}---has ample capacity to
memorise the set with no way to validate that it generalised, so a
low-parameter, interpretable cascade is the appropriate inductive
choice here, not a shortcut. With that framing:
