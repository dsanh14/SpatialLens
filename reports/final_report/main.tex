...
pedestrian relying on an assistive device, that context is exactly
what is missing. The safety-critical signal is not the class label but
the \emph{motion}: whether the agent is closing, crossing the path,
or merely present.
...
Videos are sampled at 2\,fps, resized to 960\,px wide. YOLOv8n emits
per-frame detections at conf~$\ge\!0.35$. A per-class IoU+centroid
tracker matches greedily by
...
compensate two ways: \textbf{flow} -- Farneback dense flow between
frames, minus the per-frame median flow vector
(background-dominated), giving per-bbox
$(\textrm{flow\_dx},\textrm{flow\_dy},\textrm{flow\_mag})$
relative to the scene; \textbf{frame diff} -- align the previous
frame to the current with \texttt{cv2.findTransformECC}
(translation) before differencing. Per-track aggregates: start-to-end
...
fragments. Results are not single-video luck: 6 of 11 videos are
perfect, the worst absolute count is IMG\_4982 (11/16 raw); a
per-video and motion-feature-space breakdown is in
\cref{app:dataset,app:qual}.
...
\begin{figure}[t]
  ...
  ByteTrack underperforms on sparse low-fps footage. Full numerics
  in \cref{app:detail}.}
  \label{fig:ablations}
\end{figure}
...
\label{sec:bytetrack}
ByteTrack reaches only 32.8\% even after tuning. Its Kalman-gated
...
\emph{short\_track} (23 of 24 abstentions): uncertainty is the
tracker-fragmentation tax, made auditable, not classifier ambiguity.
...
off-diagonal mass in \cref{fig:cm}. A full sub-reason breakdown, an
error taxonomy of the five root causes, and the static-class behaviour
(F1 0.00 driven by 2 one-frame parked-bicycle labels on which the
cascade correctly abstains) appear in \cref{app:failure}.
...
\Cref{fig:ablations} and the full ablation tables in
\Cref{app:detail}; \texttt{make\_final\_figures.py} produces
\Cref{fig:cm,fig:temporal,fig:scatter,fig:qual,fig:uncert} and all
booktabs tables); recorded the 11 controlled walkway videos and
hand-labelled all 61 ground-truth tracks.
...
tables (\S\ref{app:dataset}); per-video and full-ablation tables
(\S\ref{app:detail}); the 9 label-consolidation drops with the
...
\paragraph{Per-video accuracy.} Table~\ref{tab:pervideo} reports
results per video against both label sets. Of the eleven videos, six
are classified perfectly; the worst (IMG\_4982) has 16 labeled
tracks, of which 3 are 1-frame approaching fragments and 2 are
1-frame parked-bicycle ``static'' labels --- regimes where one
detection genuinely cannot resolve the intent. IMG\_4973 contains
the two trajectory-reversal errors (a monotone-bbox-growth edge
case where the reversal rule's $\rho>1.5$ threshold does not fire).
...
\paragraph{Full ablation table.} Table~\ref{tab:abl_full} is the
numeric backing of the bar chart in the main paper, with macro-F1
columns added.
...
\paragraph{Uncertainty breakdown.} \Cref{fig:uncert} shows the
sub-reason every \texttt{uncertain} prediction carries. The
distribution is heavily concentrated on \texttt{short\_track}: 23 of
the 24 abstentions are because the underlying track has $<\!3$ frames,
only 1 is \texttt{low\_signal}. Uncertainty in SpatialLens is the
tracker-fragmentation tax made auditable, not classifier ambiguity.

\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{uncertainty_breakdown.pdf}
  \caption{Sub-reason breakdown for all \texttt{uncertain} predictions.}
  \label{fig:uncert}
\end{figure}
...
\paragraph{Error taxonomy.} \Cref{tab:errors} groups the residual
errors into five root causes with example tracks and proposed fixes.
Two observations: the dominant failure is \emph{fragmentation, not
misclassification} (short-track + static abstentions account for 5 of
the 10 residuals); and \emph{no residual is in the safety-critical
asymmetry} -- the
\texttt{approaching}$\leftrightarrow$\texttt{moving\_away} confusion
is zero, and the only directional mistake (IMG\_4973/person\_4
\texttt{moving\_away}$\to$\texttt{approaching}) is the
monotone-bbox-growth edge case the reversal rule misses
($\rho\!=\!1.17 < 1.5$ threshold).
